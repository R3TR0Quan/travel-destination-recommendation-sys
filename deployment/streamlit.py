...
class RecommenderSystem:
    def __init__(self, clean_df, cosine_similarities, indices):
    ...
    def recommend_attraction(self, rating_threshold):
        ...
        recommendations = self.clean_df[self.clean_df['rating'] > rating_threshold][['name', 'LowerPrice', 'UpperPrice', 'type']]
...
def recomm_amenities(query, cosine_sim2, clean_df):
    # Check if the specified amenity exists in the dataset
    if query not in clean_df['amenities'].str.join(', '):
        print(f"Error: '{query}' does not exist in the dataset.")
        return None

    # Convert the string representation of amenities back into a list
    clean_df['amenities'] = clean_df['amenities'].apply(lambda x: ast.literal_eval(x) if isinstance(x, str) else x)

    # Get the index of the specified amenity
    indices = clean_df['amenities'].apply(lambda x: query in x if isinstance(x, list) else False)

    # Get the pairwise similarity scores of all items with the specified amenity
    sim_scores = cosine_sim2[indices]
    ...
    recommended_items = clean_df.iloc[indices]
...
recommender = RecommenderSystem(clean_df, cosine_similarities, indices)
