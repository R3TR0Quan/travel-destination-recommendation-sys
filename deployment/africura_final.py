...
def load_data():
    ...
    # Load the RecommendationEngine instance
    with open(r'../data/engine.pkl', 'rb') as f:
        engine = pickle.load(f)

    return clean_df, tfidfv_matrix2, cosine_sim2, cosine_similarities, indices, engine
...
class RecommendationEngine:
    ...
    def get_recommended_amenities(self, amenity):
        recommended_amenities = self.recommend_amenities(amenity)
...
# Load the data
clean_df, tfidfv_matrix2, cosine_sim2, cosine_similarities, indices, engine = load_data()
...
def main():
    ...
    if selection == "Recommenders":
        display_recommenders_section()


def display_recommenders_section():
    ...
    if st.button("Get Recommendations by Rating"):
        recommendations = engine.recommend_attraction(rating_threshold)
        st.dataframe(recommendations)

    place_name = st.text_input("Enter Place Name")

    if st.button("Get Recommendations by Place"):
        recommended_places = engine.recommend_place(place_name)
    ...
    amenities_data = engine.recommend_country(clean_df)
